...
-(CGSize) collectionView:(UICollectionView*)collectionView sizeWithIndex:(NSInteger)index {
    ...
    if (self.toolbarDelegate && [self.toolbarDelegate respondsToSelector:@selector(autoCompletionToolbar:cellSizeWithObject:)]) {
        ...
        CGSize size = [title boundingRectWithSize:CGSizeMake(MAX_LABEL_WIDTH, 44) options:(NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:13]} context:nil].size;
        return CGSizeMake(size.width + 13.0, 44);
    }
}
