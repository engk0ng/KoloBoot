...
RestClient::Connection::GetInfo() {
  ...
  ret.uriProxy = this->uriProxy;
    ...
    ret.pathDownload = this->pathLocDownload;
...
void
RestClient::Connection::SetPathDownload(const std::string &pathDownload) {
    this->pathLocDownload = pathDownload;
}
/**
...
RestClient::Connection::performCurlRequest(const std::string& uri) {
  ...
  /** set callback function */
    if (this->pathLocDownload.length() <= 0) {
        ...
        curl_easy_setopt(this->curlHandle, CURLOPT_WRITEFUNCTION, Helpers::write_data);
  ...
  if (!this->caInfoFilePath.empty()) {
    curl_easy_setopt(this->curlHandle, CURLOPT_CAINFO,
      ...
      curl_easy_setopt(this->curlHandle, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2);
      curl_easy_setopt(this->curlHandle, CURLOPT_SSLVERSION, CURL_SSLVERSION_DEFAULT);
      curl_easy_setopt(this->curlHandle, CURLOPT_SSL_CIPHER_LIST, "ALL");
...
RestClient::Connection::download(const std::string &uri) {
    fp = fopen(this->pathLocDownload.c_str(), "w");
    if (fp == NULL) {
        perror("fopen");
    }
